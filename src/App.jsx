...
function _Dashboard_REMOVED({ onLogout }) {
  ...
  const loadTickets = async () => {
    try {
      const res = await sbFetch("tickets?select=*,socios(nombre,direccion,telefono)&order=created_at.desc");
  ...
  const cambiarEstado = async (id, estado) => {
    ...
    showToast(`Ticket actualizado: ${estado.replace("_", " ")}`);
  };

  const filtered = tickets.filter(t => {
    const matchFilter = filter === "todos" ? true : filter === "alta" ? t.prioridad === "alta" : t.tipo === filter;
    const q = search.toLowerCase();
    const matchSearch = !q || t.resumen?.toLowerCase().includes(q) || t.socios?.nombre?.toLowerCase().includes(q) || t.tipo?.toLowerCase().includes(q);
  ...
  const stats = {
    ...
    compras: tickets.filter(t => t.tipo === "compra").length,
    medica: tickets.filter(t => t.tipo === "medica").length,
    alta: tickets.filter(t => t.prioridad === "alta").length,
  };

  const tipoBadgeStyle = (tipo) => {
    const map = { compra: { bg: "#EAF3DE", color: "#27500A" }, general: { bg: "#E6F1FB", color: "#0C447C" }, medica: { bg: "#FAEEDA", color: "#633806" }, seguimiento: { bg: "#EEEDFE", color: "#3C3489" }, asociarse: { bg: "#FBEAF0", color: "#72243E" }, consulta: { bg: "#F1EFE8", color: "#444441" } };
    return map[tipo] || map.consulta;
  };

  const prioDot = { alta: "#E24B4A", media: "#EF9F27", baja: "#639922" };

  const filters = ["todos", "compra", "medica", "general", "seguimiento", "alta"];
  const filterLabel = { todos: "Todos", compra: "Compras", medica: "Médica", general: "General", seguimiento: "Seguimiento", alta: "Alta prioridad" };

  return (
    <div style={{ minHeight: "100vh", background: "#F4FAF6", fontFamily: F }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Roboto:wght@400;500;700&display=swap'); * { box-sizing: border-box; margin: 0; padding: 0; } input, select { font-family: inherit; }`}</style>

      {/* Header */}
      <div style={{ background: C.greenDark, padding: `14px ${isMobile ? "4%" : "6%"}`, display: "flex", alignItems: "center", gap: 16 }}>
        <img src="/logo.png" alt="Cogollos" style={{ height: isMobile ? 28 : 36, filter: "brightness(0) invert(1)" }} />
        <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 13, marginLeft: 4 }}>Panel de gestión</div>
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 16 }}>
          {lastUpdate && <span style={{ color: "rgba(255,255,255,0.45)", fontSize: 12 }}>Actualizado {lastUpdate}</span>}
          <button onClick={() => { loadTickets(); }} style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", borderRadius: 8, padding: "6px 12px", fontSize: 13, cursor: "pointer", fontFamily: F }}>Actualizar</button>
          <button onClick={() => { sessionStorage.removeItem("cogo_admin"); onLogout(); }} style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.2)", color: "rgba(255,255,255,0.7)", borderRadius: 8, padding: "6px 12px", fontSize: 13, cursor: "pointer", fontFamily: F }}>Salir</button>
        </div>
      </div>

      <div style={{ maxWidth: 1200, margin: "0 auto", padding: isMobile ? "24px 4%" : "32px 6%" }}>

        {/* Stats */}
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2, 1fr)" : "repeat(4, 1fr)", gap: 12, marginBottom: 28 }}>
          {[
            { label: "Total tickets", value: stats.total, color: C.dark },
            { label: "Compras", value: stats.compras, color: C.green },
            { label: "Médica", value: stats.medica, color: "#8C6B1A" },
            { label: "Alta prioridad", value: stats.alta, color: "#991B1B" },
          ].map(s => (
            <div key={s.label} style={{ background: C.white, borderRadius: 12, border: `1px solid ${C.border}`, padding: "16px 20px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
          {filters.map(f => (
            <button key={f} onClick={() => { setFilter(f); setPage(1); }} style={{ padding: "6px 16px", borderRadius: 20, fontSize: 13, cursor: "pointer", fontWeight: 500, border: filter === f ? "none" : `1px solid ${C.border}`, background: filter === f ? C.greenDark : C.white, color: filter === f ? "#fff" : C.body, fontFamily: F }}>
              {filterLabel[f]}
            </button>
          ))}
          <input
            ...
            placeholder="Buscar..."
            style={{ marginLeft: "auto", border: `1px solid ${C.border}`, borderRadius: 8, padding: "7px 14px", fontSize: 13, color: C.dark, background: C.white, outline: "none", width: isMobile ? "100%" : 200 }}
          />
        </div>

        {/* Tickets */}
        {loading ? (
          <div style={{ textAlign: "center", padding: "60px 0", color: C.muted, fontSize: 15 }}>Cargando tickets...</div>
        ) : paginated.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 0", color: C.muted, fontSize: 15 }}>No hay tickets con ese filtro</div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {paginated.map(t => {
              const fecha = new Date(t.created_at).toLocaleDateString("es-AR", { day: "2-digit", month: "2-digit", year: "2-digit", hour: "2-digit", minute: "2-digit" });
              const tipo = t.tipo || "consulta";
              const badge = tipoBadgeStyle(tipo);
              const socio = t.socios?.nombre || "—";
              const dir = t.socios?.direccion || "";
              return (
                <div key={t.id} style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 12, padding: "14px 18px", display: "flex", alignItems: isMobile ? "flex-start" : "center", gap: 12, flexDirection: isMobile ? "column" : "row" }}>
                  {/* Dot prioridad */}
                  <div style={{ width: 9, height: 9, borderRadius: "50%", background: prioDot[t.prioridad] || prioDot.media, flexShrink: 0, marginTop: isMobile ? 4 : 0 }} title={`Prioridad ${t.prioridad}`} />

                  {/* Cuerpo */}
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 500, color: C.dark, marginBottom: 5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.resumen || "(sin resumen)"}</div>
                    <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center", fontSize: 12, color: C.muted }}>
                      <span style={{ background: badge.bg, color: badge.color, borderRadius: 20, padding: "2px 10px", fontWeight: 500 }}>{tipo}</span>
                      <span>{socio}</span>
                      {dir && <span>{dir}</span>}
                      <span>{fecha}</span>
                    </div>
                  ...
                  <select
                    ...
                    style={{ border: `1px solid ${C.border}`, borderRadius: 8, padding: "6px 10px", fontSize: 13, color: C.dark, background: C.white, cursor: "pointer", fontFamily: F, flexShrink: 0 }}
                  >
        ...
        {totalPages > 1 && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 20 }}>
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} style={{ padding: "6px 14px", borderRadius: 8, border: `1px solid ${C.border}`, background: C.white, cursor: page === 1 ? "not-allowed" : "pointer", color: C.body, fontFamily: F }}>‹</button>
            <span style={{ fontSize: 13, color: C.muted }}>Página {page} de {totalPages} · {filtered.length} tickets</span>
            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} style={{ padding: "6px 14px", borderRadius: 8, border: `1px solid ${C.border}`, background: C.white, cursor: page === totalPages ? "not-allowed" : "pointer", color: C.body, fontFamily: F }}>›</button>
          </div>
        )}
      </div>
      ...
      {/* Toast */}
      {toast && (
        <div style={{ position: "fixed", bottom: 24, right: 24, background: C.greenDark, color: "#fff", padding: "10px 18px", borderRadius: 10, fontSize: 13, fontFamily: F, boxShadow: "0 4px 16px rgba(0,0,0,0.15)", zIndex: 999 }}>{toast}</div>
      )}
    </div>
  );
}
